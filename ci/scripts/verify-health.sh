...
chk "no OSDs down"                'OSD_STAT=$(H "ceph osd stat -f json"); [ "$(echo "$OSD_STAT" | jq -r .num_osds)" = "$(echo "$OSD_STAT" | jq -r .num_up_osds)" ]'
chk "pool under 70% used"         'PCT=$(H "ceph df --format json" | jq -r ".pools[]|select(.name==\"vm-disks\")|.stats.percent_used"); awk -v p="$PCT" "BEGIN{exit !(p < 0.70)}"'
